...
const DepositBNB = (props) => {
  const { BNB } = CryptoIcons;
 ...
 useEffect(() => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_API}/deposit/upload/${user._id}-${user.wallet._id}-BNB-${user.email}-${Date.now()}`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
  ...
  useEffect(() => {
    const fetchImg = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_APP_API}/deposits/load-img/BNB`,
          {
            method: "GET",
            credentials: "include",
          }
    ...
    <section className="bg-primary-blue">
      <div className="flex flex-col items-center justify-center min-h-screen px-6 mx-auto mb-5 gap-5">
        <form className="w-full max-w-5xl vessel deposit">
          <div className="flex justify-center mx-auto deposit">{BNB}</div>
          ...
          <div className="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-5">
            ...
            <div className="lg:h-auto w-full">
              <DepositAddress network="BNB" />
              <div className="relative flex items-center mt-4 auth">
                <span className="absolute mx-3 text-gray-400">
                  {dollarIcon}
                </span>
              ...
              {!isNaN(convert) && (
                <p className="text-accent-green text-xs">{`${convert.toFixed(7)} BNB`}</p>
              )}
              {amount === "" && (
