...
const DepositBTC = (props) => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_API}/deposit/upload/${user?._id}-${user?.wallet._id}-BTC-${user?.email}-${Date.now()}`,
        {
          method: "POST",
          credentials: "include",
          body: formData,
  ...
  useEffect(() => {
    const fetchImg = async () => {
      try {
        const response = await fetch(
          `${import.meta.env.VITE_APP_API}/deposits/load-img/BTC`,
          {
            method: "GET",
            credentials: "include",
          }
    ...
    <section className="bg-primary-blue">
      <div className="flex flex-col items-center justify-center min-h-screen px-6 mx-auto mb-5 gap-5">
        <form className="w-full max-w-5xl vessel deposit">
          <div className="flex justify-center mx-auto deposit">{BTC}</div>
